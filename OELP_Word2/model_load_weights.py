...
class Model:
    ...
    def setup_cnn(self) -> None:
        """Create CNN layers."""
        is_trainable = False
        val_in = tf.expand_dims(input=self.input_imgs, axis=3)
        ...
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        ...
        # LAYER 2
        la = saved_model.get_layer(name='conv2d_1')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        ...
        # LAYER 3
        la = saved_model.get_layer(name='conv2d_2')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=val_in, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 4
        la = saved_model.get_layer(name='conv2d_3')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 5
        la = saved_model.get_layer(name='conv2d_4')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=val_in, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 6
        la = saved_model.get_layer(name='conv2d_5')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 7
        la = saved_model.get_layer(name='conv2d_6')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 8
        la = saved_model.get_layer(name='conv2d_7')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        la = saved_model.get_layer(name='conv2d_8')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 10
        la = saved_model.get_layer(name='conv2d_9')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 11
        la = saved_model.get_layer(name='conv2d_10')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 12
        la = saved_model.get_layer(name='conv2d_11')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        # LAYER 13
        la = saved_model.get_layer(name='conv2d_12')
        out = la.get_weights()
        kernel,bias = tf.Variable(out[0], trainable = is_trainable),tf.Variable(out[1], trainable = is_trainable)
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv = tf.nn.bias_add(conv, bias)
        ...
        relu = tf.nn.max_pool2d(input=relu, ksize=(1, 1,2, 1),strides=(1, 1,2, 1), padding='VALID')
        relu = tf.nn.max_pool2d(input=relu, ksize=(1, 1,2, 1),strides=(1, 1,2, 1), padding='VALID')
    ...
    def setup_rnn(self) -> None:
        ...
        self.rnn_out_3d = tf.squeeze(tf.nn.atrous_conv2d(value=concat, filters=kernel, rate=1, padding='SAME'),
                                     axis=[2])
        print("rnn output shape : ",self.rnn_out_3d.shape)
...
#model = Model("ffhjejgijr", "bestpath")