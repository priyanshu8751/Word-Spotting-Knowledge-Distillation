...
class Model_teacher:
    ...
    def setup_cnn(self) -> None:
        ...
        # LAYER 1
        kernel = tf.Variable(tf.random.truncated_normal([3,3,3,64],stddev=0.1))
        ...
        # LAYER 11
        kernel = tf.Variable(tf.random.truncated_normal([3,3,256,512],stddev=0.1))
        conv = tf.nn.conv2d(input=relu, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv_norm = tf.compat.v1.layers.batch_normalization(conv, training=self.is_train)
        relu = tf.nn.relu(conv_norm)
    ...
    def setup_rnn(self) -> None:
        ...
        print("RNN rnn_in3d shape : ",rnn_in3d.shape)
...
class Model_student:
    ...
    def setup_cnn(self) -> None:
        ...
        # LAYER 1
        kernel = tf.Variable(tf.random.truncated_normal([3,3,3,32],stddev=0.1))
        conv = tf.nn.conv2d(input=val_in, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv_norm = tf.compat.v1.layers.batch_normalization(conv, training=self.is_train)
        relu = tf.nn.relu(conv_norm)
