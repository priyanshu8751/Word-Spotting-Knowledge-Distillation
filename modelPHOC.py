...
class Model:
    ...
    def setup_cnn(self) -> None:
        ...
        # maxpool = tf.nn.max_pool2d(input=relu, ksize=2,strides=2,padding="VALID")
        ...
        conv = tf.nn.conv2d(input=maxpool, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        conv_norm = tf.compat.v1.layers.batch_normalization(conv, training=self.is_train)
        relu = tf.nn.relu(conv_norm)
        ...
        kernel = tf.Variable(tf.random.truncated_normal([1,1,64,128],stddev=0.1))
        ...
        kernel = tf.Variable(tf.random.truncated_normal([1,1,128,256],stddev=0.1))
        x_skip = tf.nn.conv2d(input=x_skip, filters=kernel, padding='SAME', strides=(1, 1, 1, 1))
        x_skip = tf.compat.v1.layers.batch_normalization(x_skip, training=self.is_train)
        relu = tf.nn.relu(tf.add(x_skip,conv_norm))
        # block ends count 1
        ...
        kernel = tf.Variable(tf.random.truncated_normal([1,5,256,256],stddev=0.1))
